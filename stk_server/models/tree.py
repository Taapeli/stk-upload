...
def create_tree(result, field_id='name', field_type='type'):
    ...
    for record in result:
        ...
        for node in record['nodes']:
            if not node.id in nl:
                nl[node.id] = node[field_id]                #["pname"]
                nstack.append((node.id, node[field_type],   #["type"], 
                               node[field_id],              #["pname"], 
                               record["lv"]))
        for rel in record['r']:
...
if __name__ == '__main__':
    ...
    tree = create_tree(neo_result, 'pname', 'type')
    print(tree)
    print_tree(tree)
#     print(tree.to_json())
