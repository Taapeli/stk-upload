...
#         for node_n in n_nodes:
#             n = Note.from_node(node_n)
...
#         node_near = record['x']
#         x_uid = node_near.id    # Nearest referring node (Event, Person, Name, ...)
# 
#         noderef = NodeRef()
#         noderef.uuid = root_uuid      # 72104
#         noderef.uniq_id = root_uniq_id      # 72104
#         noderef.id = node_near['id']    # 'I1069' or 'E2821' TODO Why?
#         noderef.label = root_label
#         noderef.node_far = record['p']    # node for Person or Family etc
# 
#         event_role = record.get('role', "")
#         print(f'{root_label} {root_uniq_id} Citation {c.uniq_id} {c.id} {noderef.label}({event_role}) {noderef.uniq_id} {noderef.id}')
# 
#         noderef.person = None
#         noderef.family = None
#         if 'Person' in noderef.node_far.labels:
#             noderef.person = Person_combo.from_node(noderef.node_far)
#         if 'Family' in noderef.node_far.labels:
#             noderef.family = Family.from_node(noderef.node_far)
#             
#         if noderef.label == "Person":
#             pass
#         elif noderef.label == "Family":
#             noderef.eventtype = _(noderef.node_far['type'])
