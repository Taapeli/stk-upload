...
#         return ret
    ...
    def _get_surname_list(self,surn):
    ...
    def xxx_get_surname_list(self,surn):
        TYPES = {'os.':'syntymänimi', 
                 'o.s.':'syntymänimi', 
                 'ent.':'otettu nimi', 
                 'e.':'otettu nimi', 
        ...
        #surname = Optional(prefix) + Word(alphas)
        #surnamegroup = surname + Optional
        name_type = None
        prefix = None
        name = None
        ret = []
        for nm in surn.split():
            if nm in TYPES:
                if name: 
                    ret.append( (prefix,name,name_type) )
                    name_type = None
                ...
                name_type = TYPES.get(nm)
                continue
            if len(nm) < 4 and not '.' in nm: # von
                if name: 
                    ret.append( (prefix,name,name_type) )
                    name_type = None
                    prefix = None
            ...
            if name: 
                ...
                name = nm
        if name: ret.append( (prefix,name,name_type) )
        if len(ret) > 1 and ret[0][2] is None: ret[0] = (ret[0][0], ret[0][1], "avionimi")
        return ret
...
def test():
    ...
    test_givn("(Matti) Pekka")
    test_givn("(Matti)")
    test_givn("Matti Pekanpoika")
    test_givn("poikalapsi")
    test_givn("Johan Henrici")

    test_surn("Mattila")
    test_surn("Frisk os. Mattila")
    test_surn("Reipas e. Frisk os. Laine")
    test_surn("Lehti os. Lampi e. af Damm")
    test_surn("Mattila (Matts)")
