...
def record_with_subs(oid):
    print(f"Getting record {oid} with subs")
    result = shareds.driver.session().run(cypher_record,id=oid).single()
    print(result)
...
def record_with_selected_subs(oid, subs):
    print(f"Getting records {oid} with subs {subs}")
    result = shareds.driver.session().run(cypher_record_with_selected_subs, id=oid, subs=subs)
    print(result)
    if not result: return dict(status="Not found",statusText="Not found",resultCount=0)
    p = result.get('p')
#    print(f"id={p['id']} name={p['pname']}")
...
def selected_records_with_subs(oids):
    print(f"Getting records {oids} with subs")
    result = shareds.driver.session().run(cypher_selected_records_with_subs, id=oids)
    print(result)
    if not result: return dict(status="Not found",statusText="Not found",resultCount=0)
    p = result.get('p')
#    print(f"id={p['id']} name={p['pname']}")
